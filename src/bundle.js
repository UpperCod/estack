...
function getPages(page, filesHtml, groupOrder) {
  const link = path.join("./", page.folder || "", page.link);
  ...
  const orderFn = ([a], [b]) => (a > b ? 1 : -1);

  // Create an indexable map, these are grouped by group
  const [fistGroup, lastGroup] = [
    ...
    .sort(orderFn)
    .reduce(
      //Create 2 groups the first group those opted by orderGroup and the second the remaining
      ([firstGroup, lastGroup], item) => {
        let position = groupOrder.indexOf(item[0]);
        if (position > -1) {
          firstGroup.set(position, item);
        } else {
          lastGroup.push(item);
        }
  ...
  // join the groups
  return [
    ...[...fistGroup].sort(orderFn).map(([, item]) => item),
    ...lastGroup
  ].map(([group, items]) => ({
    group,
